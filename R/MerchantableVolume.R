...
# dbh = diameter (cm) at breat height of tree i
# ht = height (m) of tree
# bj, j=0, 1, 2 are parameters of the equation

# Using parameters from Table 11 in North et al. 2016
...
for(i in 1:nrow(B1trees)){
  t[i] <- TreeMerchVolFN(Species=B1trees[i,Species], DBH=B1trees[i,DBH], 
                       HT=B1trees[i,Height])
}
