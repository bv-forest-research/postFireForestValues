...
PlotMarten <- merge(plot_treatments, PlotCQI)
...
# 1. CWD quality
PlotFisher <- merge(plot_treatments, PlotCQI)
...
PlotGoshawk <- merge(plot_treatments, PlotSnags[DBH_bin >= 30, .(snagSPH=sum(snagSPH)),
                                                by=PlotID], all.x=TRUE)
PlotGoshawk <- merge(PlotGoshawk, PlotTree[DBH_bin >= 30, .(liveSPH=sum(SPH)), by=PlotID], 
                     all.x=TRUE)
...
PlotGoshawk[, GoshawkHabitat := round(sum(2*retention, 0.8*CQI, 0.7*PileCount, 0.8*ShrubsB1),
                                      digits = 2), by=PlotID]
...
PlotHare <- merge(plot_treatments, PlotCrown, all.x = TRUE)
...
PlotHare[, HareHabitat := round(sum(1*CrownClos, 1*SPH, 0.8*lowShrubs, 0.6*CWDvol), digits = 2),
         by=PlotID]
...
PlotSquirrel <- merge(plot_treatments, PlotTree[DBH_bin >= 20, .(SPH = sum(SPH)), by=PlotID], 
                      all.x = TRUE)
...
PlotSmMammal <- merge(plot_treatments, PlotCWD[Decay_class >= 3, .(LateCWDvol= sum(VolHa)),
                                             by=c("PlotID")], all.x = TRUE) # maybe take out class 5 if others also don't include it
...
PlotGrouse <- merge(plot_treatments, PlotCrown, all.x = TRUE)
...
PlotGrouse[, GrouseHabitat := round(sum(CrownClos, conifer, deciduous, ShrubsB1), digits = 2),
           by=PlotID]
...
PlotGrizzly <- merge(plot_treatments, PlotCQI)
