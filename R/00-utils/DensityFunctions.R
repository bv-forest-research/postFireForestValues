...
TreeDensity <- function(treeDat_a1,treeDat_b1, ClassSize){
  ...
  FR_LiveTrees <- rbind(A1trees[Tree_class<3,.(PlotID, `Sub-plot`, 
                                               Species, Tree_class, DBH, Height, Notes)],
                        B1trees[Tree_class<3,.(PlotID, `Sub-plot`, 
                                               Species, Tree_class, DBH,Height)], fill=TRUE)
  ...
  for(j in 1:length(diamClasses)){
    #the diameter class is the upper bound
    FR_LiveTrees[DBH <= diamClasses[j] & DBH > diamClasses[j]-dbhClSize, 
                 DBH_bin := diamClasses[j]]
  }
...
SnagDensity <- function(treeDat_a1,treeDat_b1){
  ...
  #UniSpecies <- unique(c(A1trees[Tree_class<3,Species],B1trees[Tree_class<3,Species],
  #                      Regen[`Live/Dead`=="L",Species]))#Species in plots:
  FR_DeadTrees <- rbind(A1trees[Tree_class>2,.(PlotID, `Sub-plot`, Species, Tree_class, DBH, Height, Notes)],
                        B1trees[Tree_class>2,.(PlotID, `Sub-plot`, Species, Tree_class, DBH,Height)], fill=TRUE)
  FR_DeadTrees <- FR_DeadTrees[!is.na(DBH)]
  #deal with the different plot sizes and areas surveyed
  FR_DeadTrees[,AreaSearchM2 := ifelse(`Sub-plot`=="A1",100, ifelse(`Sub-plot`=="A2",50,400))]
  ...
  FR_DeadTrees[,PlotID:=as.factor(PlotID)][,Species:=as.factor(Species)]
  PlotTrees <- FR_DeadTrees[,.(snagSPH=sum(PHF)),by=c("PlotID","Species","DBH_bin","Height")] #only 1 plot, sum up stems in each diam class
  return(PlotTrees)
}
